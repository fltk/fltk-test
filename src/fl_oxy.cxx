...
static void arrow_begin(bool ac, Fl_Color c, Fl_Color hc, float av) {
  Fl_Color col = fl_color_average(c, hc, av);
  fl_color(ac ? col : fl_inactive(col));
  fl_begin_line();
}
...
static void single_arrow(Fl_Rect bb, Fl_Orientation o,
  ...
  arrow_begin(ac, c, hc, 0.38);
  fl_vertex(x0, -dx);
  fl_vertex(x0 + dx, 0);
  fl_vertex(x0, dx);
  arrow_end();

  arrow_begin(ac, c, hc, 0.58);
  fl_vertex(x0, -dx + 1);
  fl_vertex(x0 + dx - 1, 0);
  fl_vertex(x0, dx - 1);
  arrow_end();

  arrow_begin(ac, c, hc, 0.78);
  fl_vertex(x0 + 1, -dx);
  fl_vertex(x0 + dx + 1, 0);
  fl_vertex(x0 + 1, dx);
  arrow_end();
...
void oxy_arrow(Fl_Rect bb, Fl_Arrow_Type t, Fl_Orientation o,
  ...
  switch(t) {
    ...
    case FL_ARROW_CHOICE:
      ...
      bb.y(bb.y() - 2); // shift upwards
      bb.h(bb.h() - 4); // reduce size
      single_arrow(bb, FL_ORIENT_DOWN, ac, c, hc);
      bb.y(bb.y() + 4); // shift down
      single_arrow(bb, FL_ORIENT_DOWN, ac, c, hc);
      break;
...
static Fl_Color oxy_color(Fl_Color c) {
...
static void oxy_draw(int x, int y, int w, int h, Fl_Color c, int typebox, bool is_shadow) {
  ...
  if (typebox != _FL_OXY_UP_FRAME && typebox != _FL_OXY_DOWN_FRAME) {
    ...
    switch (typebox) {
      case _FL_OXY_UP_BOX: {
	_oxy_up_box_(X, Y, W, H, oxy_color(c));
	break;
      }
      case _FL_OXY_DOWN_BOX: {
	_oxy_down_box_(X, Y, W, H, oxy_color(c));
	break;
      }
      case _FL_OXY_BUTTON_UP_BOX: {
	_oxy_button_up_box_(X, Y, W, H, oxy_color(c));
	break;
      }
      case _FL_OXY_BUTTON_DOWN_BOX: {
	_oxy_button_down_box_(X, Y, W, H, oxy_color(c));
	break;
      }
      case _FL_OXY_ROUND_UP_BOX:
      case _FL_OXY_ROUND_DOWN_BOX:
	_oxy_rounded_box_(x, y, w, h, oxy_color(fl_color_average(c, FL_WHITE, 0.82)));
	break;
      default: { break; }
    }
  }
  ...
  if (typebox == _FL_OXY_ROUND_UP_BOX || typebox == _FL_OXY_ROUND_DOWN_BOX) {
    ...
    topline = leftline = rightline = bottomline = fl_color_average(c, FL_BLACK, 0.85);
  }
...
void button_up_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_BUTTON_UP_BOX, true);
}
void button_down_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_BUTTON_DOWN_BOX, true);
}
void up_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_UP_BOX, true);
}
void down_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_DOWN_BOX, true);
}
void thin_up_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_UP_BOX, false);
}
void thin_down_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_DOWN_BOX, false);
}
void up_frame(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_UP_FRAME, true);
}
void down_frame(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_DOWN_FRAME, true);
}
void thin_up_frame(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_UP_FRAME, false);
}
void thin_down_frame(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_DOWN_FRAME, false);
}
void round_up_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_ROUND_UP_BOX, true);
}
void round_down_box(int x, int y, int w, int h, Fl_Color c) {
  oxy_draw(x, y, w, h, c, _FL_OXY_ROUND_DOWN_BOX, true);
}
